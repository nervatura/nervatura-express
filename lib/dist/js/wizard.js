...
function getUrl(urlfunc){
  ...
  url = url.replace("@code", "code="+((document.getElementById("code").value === "") ? "none" : document.getElementById("code").value));
  return url;}
...
function getParams(urlfunc){
  ...
  if (document.getElementById("code")) {
    if (document.getElementById("code").value==="base64") {
      var bytes = new (TextEncoder || TextEncoderLite)("utf-8").encode(params);        
      params = "params="+base64js.fromByteArray(bytes);
    } else {
...
function createView(){
  ...
  if (document.getElementById("datatype").value!="sql") {
	...
	if (document.getElementById("code").value=="base64") {
    var bytes = new (TextEncoder || TextEncoderLite)("utf-8").encode(filters);        
    filters = "filters="+base64js.fromByteArray(bytes); } 
	else {
	  filters = "filters="+filters;}
	url = url.replace("@params", params);
	url = url.replace("@data", filters);
	document.getElementById("rs_url").value = encodeURI(url);
...
function createUpdate(itype){
  ...
  for (var i in inputs) {
	...
	if (document.getElementById("code").value=="base64") {
    var bytes = new (TextEncoder || TextEncoderLite)("utf-8").encode(data.substring(2));        
    data = "data="+base64js.fromByteArray(bytes); } 
  else {
